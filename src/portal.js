...
if (/^YV[A-F0-9]{8}$/.test(referralFromUrl)) {
  S.referralCode = referralFromUrl;
  try { localStorage.setItem("yavoi-referral-code", referralFromUrl); } catch {}
} else {
  try { S.referralCode = String(localStorage.getItem("yavoi-referral-code") || "").trim().toUpperCase(); } catch {}
}
const modal = $("#modal");
...
function authPage(view = "login", message = "") {
  ...
  bindForm("#auth-form", async (v) => {
    if (signup) {
      const referralCode = String(v.referral_code || "").trim().toUpperCase();
      if (referralCode) {
        S.referralCode = referralCode;
        try { localStorage.setItem("yavoi-referral-code", referralCode); } catch {}
      }
...
function onboarding() {
  ...
  $$('#onboard input[name="role"]').forEach((input) => input.onchange = () => {
    $("#onboard-referral").classList.toggle("hidden", input.value === "driver" && input.checked);
  });
  bindForm("#onboard", async (v) => {
    await rpc("onboard", { ...v, referral_code: v.role === "passenger" ? String(v.referral_code || "").trim().toUpperCase() : "" });
    try { localStorage.removeItem("yavoi-referral-code"); } catch {}
    S.referralCode = "";
    await loadSession();
  });
}
...
function shiftTimeLabel(shift) {
  if (shift.code === "all_day") return "Disponible las 24 horas";
...
function openRewardCoupon(redemption) {
  ...
  const claimInstructions = redemption.claim_instructions || redemption.fulfillment_note || (tripBenefit
    ? "Elige esta recompensa al confirmar un próximo viaje."
    : "Presenta este ticket antes de recibir el beneficio.");
  const claimAction = redemption.claim_contact_url && !tripBenefit
    ? `<a class="btn" href="${e(redemption.claim_contact_url)}" target="_blank" rel="noopener noreferrer">${e(redemption.claim_button_label || "Contactar para canje")} ${I("send")}</a>`
    : "";
  openModal(
    usable ? (tripBenefit ? "Recompensa guardada" : "Tu ticket está listo") : "Detalle de tu recompensa",
    `<article class="reward-coupon" id="reward-coupon-card">${image ? `<img class="reward-coupon-logo" src="${e(image)}" alt="${e(redemption.partner_name || redemption.name)}">` : `<div class="reward-coupon-brand">Yavoi!</div>`}<div class="reward-coupon-copy"><span class="badge ${usable ? "" : "pending"}">${e(rewardStatusName[redemption.status] || redemption.status)}</span><small>${e(redemption.partner_name || "Yavoi!")}</small><h3>${e(redemption.name)}</h3><p>${e(redemption.description)}</p></div>${usable && !tripBenefit ? `<div class="reward-coupon-code">${rewardBarcode(redemption.code)}<strong>${e(redemption.code)}</strong><small>Código individual, irrepetible y de un solo uso</small></div>` : `<div class="reward-coupon-reference"><small>${redemption.status === "requested" ? "FOLIO DE SOLICITUD" : "CÓDIGO DE RECOMPENSA"}</small><strong>${e(redemption.code)}</strong></div>`}<div class="reward-coupon-claim"><strong>Cómo canjearla</strong><p>${e(claimInstructions)}</p>${redemption.claim_location ? `<span>${I("map-pin")} ${e(redemption.claim_location)}</span>` : ""}${redemption.claim_contact ? `<span>${I("headset")} ${e(redemption.claim_contact)}</span>` : ""}${redemption.redemption_reference ? `<span>${I("hash")} Referencia de Operaciones: ${e(redemption.redemption_reference)}</span>` : ""}</div>${redemption.terms ? `<div class="reward-coupon-terms"><strong>Condiciones</strong><p>${e(redemption.terms)}</p></div>` : ""}<div class="reward-coupon-meta"><span>Propietario: ${e(ownerName)}</span>${redemption.expires_at ? `<span>Válido hasta: ${date(redemption.expires_at)}</span>` : ""}</div></article><div class="coupon-actions">${claimAction}${usable && !tripBenefit ? `<button class="btn secondary" id="share-reward-coupon">Compartir ticket ${I("share-2")}</button>` : ""}<button class="btn secondary" id="copy-reward-code">Copiar folio ${I("copy")}</button></div><p class="hint">${tripBenefit ? "Al confirmar un viaje sólo podrás elegir una recompensa disponible; el sistema la bloqueará para evitar usos repetidos." : usable ? "Muestra el código únicamente al momento de canjear. Operaciones registrará su uso para que no pueda repetirse." : "Conserva este folio. El código de barras aparecerá cuando el beneficio esté listo para usar."}</p>`,
  );
  if ($("#share-reward-coupon")) $("#share-reward-coupon").onclick = () => run(async () => {
...
function referralShareUrl(code) {
  const url = new URL("/portal.html", location.origin);
  url.searchParams.set("ref", code);
  url.searchParams.set("signup", "1");
  return url.toString();
}
function referralShareText(code) {
  return `Te invito a viajar con Yavoi! Regístrate con mi código ${code}, completa tu perfil y solicita tu primer viaje: ${referralShareUrl(code)}`;
}
function shareReferral(code, preferWhatsApp = false) {
  return run(async () => {
    ...
    if (navigator.share) await navigator.share({ title: "Viaja con Yavoi!", text });
    else {
      await navigator.clipboard.writeText(text);
      notify("Invitación copiada para compartir.");
    }
  });
}
function rewardEligibility(reward, metrics, systemEnabled = true) {
  ...
  if (Number(metrics.referral_registered_count || 0) < Number(reward.min_referrals_registered || 0)) return [false, `Requiere ${reward.min_referrals_registered} invitaciones registradas`];
  if (Number(metrics.referral_first_trip_count || 0) < Number(reward.min_referrals_first_trip || 0)) return [false, `Requiere ${reward.min_referrals_first_trip} invitados con primer viaje`];
...
function activeRewardCard(item) {
  ...
  const action = tripBenefit ? "Elegir al confirmar viaje" : pending ? "Ver solicitud" : "Ver ticket";
  const instruction = tripBenefit
    ? "Disponible para elegir una sola vez en la confirmación de un viaje compatible."
    : item.claim_instructions || item.fulfillment_note || "Consulta el ticket para ver cómo recibir el beneficio.";
  return `<article><div><div class="row wrap"><strong>${e(item.name)}</strong><span class="badge neutral">${tripBenefit ? "Para viaje" : "Canje externo"}</span></div><small>${e(item.code)} · ${e(rewardStatusName[item.status] || item.status)}${item.expires_at ? ` · vence ${date(item.expires_at)}` : ""}</small><p>${e(instruction)}</p></div><div class="row wrap"><span class="badge ${pending ? "pending" : ""}">${e(rewardStatusName[item.status] || item.status)}</span><button class="btn secondary" data-view-coupon="${e(item.id)}">${action} ${I(tripBenefit ? "car-front" : "barcode")}</button></div></article>`;
}
function rewardHistoryCard(item) {
  const used = ["applied", "redeemed"].includes(item.status);
  return `<article><div><strong>${e(item.name)}</strong><small>${e(item.code)} · ${e(rewardStatusName[item.status] || item.status)}${item.redemption_reference ? ` · ref. ${e(item.redemption_reference)}` : ""}</small></div><div class="row wrap"><span class="badge ${used ? "" : "pending"}">${e(rewardStatusName[item.status] || item.status)}</span><button class="btn secondary" data-view-coupon="${e(item.id)}">Ver detalle ${I("file-text")}</button></div></article>`;
}
function rewards() {
  ...
  const activeBenefits = redemptions.filter((item) => ["available", "requested", "fulfilled"].includes(item.status));
  const pastBenefits = redemptions.filter((item) => ["applied", "redeemed", "cancelled", "expired"].includes(item.status));
  ...
  if (referralCode) {
    ...
    $("#copy-referral").onclick = () => run(async () => {
      await navigator.clipboard.writeText(referralCode);
      notify("Código de invitación copiado.");
    });
  }
  ...
  $$('[data-redeem]').forEach((item) => item.onclick = () => {
    ...
    const redemptionHint = reward.delivery_mode === "trip"
      ? "La recompensa quedará guardada para elegirla al confirmar un próximo viaje. Sólo se permite una por viaje."
      : reward.delivery_mode === "digital_coupon"
        ? "Tu ticket individual y código de barras aparecerán inmediatamente después del canje."
        : "Se generará un folio para Operaciones y recibirás el ticket cuando el beneficio esté preparado.";
    openModal(
      "Confirmar canje",
      `<div class="reward-confirm">${I(reward.icon || "gift")}<h3>${e(reward.name)}</h3><p>${e(reward.description)}</p><div class="receipt-row total"><span>Costo</span><strong>${reward.points_cost} puntos</strong></div><p class="hint">${e(redemptionHint)}</p><button class="btn wide" id="confirm-reward">Canjear recompensa ${I("arrow-right")}</button></div>`,
    );
...
function maybeShowRewardPromo() {
  ...
  const referralPromo = !driver && wallet.referral_code
    ? `<div class="next-reward referral-promo"><small>INVITA Y GANA</small><strong>100 puntos por cada invitado efectivo</strong><p>Gana 30 puntos al completar su registro y 70 cuando termine su primer viaje.</p><button class="btn referral-btn wide" id="promo-referral-whatsapp">${I("message-circle")} Invitar por WhatsApp</button></div>`
    : "";
  openModal(
    driver ? "Tu desempeño también suma" : "Tus Puntos Viajeros te esperan",
    `<div class="reward-welcome">${I(driver ? "star" : "gift")}<div><span class="badge">Nivel ${e(wallet.level)}</span><h3>${wallet.available_points} puntos disponibles</h3><p>${driver ? `${wallet.trip_count} viajes · ${wallet.rating ? `${decimal(wallet.rating)}/5 de rating` : "completa tus primeros viajes para formar tu rating"}.` : `${wallet.trip_count} viajes completados · ${freeRides} viajes locales gratis guardados.`}</p></div></div>${referralPromo}${nextReward ? `<div class="next-reward"><small>PRÓXIMA META</small><strong>${e(nextReward.name)}</strong><p>${Number(wallet.available_points || 0) >= nextReward.points_cost ? "Ya tienes puntos para solicitarla." : `Te faltan ${nextReward.points_cost - Number(wallet.available_points || 0)} puntos para alcanzarla.`}</p></div>` : ""}<button class="btn wide" id="open-rewards">Ver mis recompensas ${I("arrow-right")}</button>`,
  );
  if ($("#promo-referral-whatsapp")) $("#promo-referral-whatsapp").onclick = () => shareReferral(wallet.referral_code, true);
...
const DRIVER_DRAFT_CHECKBOXES = [
  "vehicle_verification_not_applicable", "seatbelts_all", "front_airbags", "abs_brakes",
  "first_service_tools", "extinguisher_abc", "four_doors", "air_conditioning",
  "reflective_markings", "advertising_interest",
];
const DRIVER_UPLOAD_FIELDS = [
  ["vehicle_front_file", "vehicle_front_path", "yavoi-vehicle-photos"],
  ["government_id_file", "government_id_path", "yavoi-documents"],
  ["license_file", "license_path", "yavoi-documents"],
  ["transport_card_file", "transport_card_path", "yavoi-documents"],
  ["insurance_file", "insurance_path", "yavoi-documents"],
  ["vehicle_registration_file", "vehicle_registration_path", "yavoi-documents"],
  ["vehicle_verification_file", "vehicle_verification_path", "yavoi-documents"],
  ["mechanical_inspection_file", "mechanical_inspection_path", "yavoi-documents"],
  ["tax_compliance_file", "tax_compliance_path", "yavoi-documents"],
  ["criminal_record_file", "criminal_record_path", "yavoi-documents"],
  ["policy_commitment_file", "policy_commitment_path", "yavoi-documents"],
  ["traffic_law_commitment_file", "traffic_law_commitment_path", "yavoi-documents"],
];
function driverDraftKey() {
  return `yavoi-driver-dossier-draft:${S.user?.id || "anonymous"}`;
}
function driverDraftSnapshot(form) {
  const values = {};
  ...
  let uploaded_paths = {};
  try { uploaded_paths = JSON.parse(form.dataset.uploadedPaths || "{}"); } catch {}
  return { version: 1, values, uploaded_paths, saved_at: new Date().toISOString() };
}
function storedDriverDraft() {
  const remote = S.data.driver_profile_draft?.draft;
  let local = null;
  try { local = JSON.parse(localStorage.getItem(driverDraftKey()) || "null"); } catch {}
  if (!remote) return local;
  if (!local) return remote;
  return Date.parse(local.saved_at || 0) > Date.parse(remote.saved_at || 0) ? local : remote;
}
function restoreDriverDraft(form) {
  const draft = storedDriverDraft();
  ...
  form.dataset.uploadedPaths = JSON.stringify(draft.uploaded_paths || {});
  const uploaded = Object.keys(draft.uploaded_paths || {}).length;
...
async function uploadDriverDraftFiles(form) {
  let uploaded = {};
  try { uploaded = JSON.parse(form.dataset.uploadedPaths || "{}"); } catch {}
  for (const [inputName, pathName, bucket] of DRIVER_UPLOAD_FIELDS) {
...
function profile() {
  ...
  if (editState.editable) {
    ...
    bindForm("#vehicle-form", async (v, f) => {
      clearTimeout(S.driverDraftTimer);
      const uploadedPaths = await uploadDriverDraftFiles(f);
      clearTimeout(S.driverDraftTimer);
      ...
      try { localStorage.removeItem(driverDraftKey()); } catch {}
      f.dataset.uploadedPaths = "{}";
      await loadSession();
  ...
  if (driver && editState.editable) {
    ...
    const updateProgress = () => {
      ...
      try { Object.assign(snapshot, JSON.parse(form.dataset.uploadedPaths || "{}")); } catch {}
    ...
    const updateAndSaveDraft = () => {
      updateProgress();
      saveDriverDraft(form);
    };
    form.addEventListener("input", updateAndSaveDraft);
    form.addEventListener("change", updateAndSaveDraft);
    updateProgress();
  }
...
const rewardClaimMethodNames = {
  trip: "Elegir durante la confirmación del viaje",
  show_ticket: "Mostrar ticket digital",
  partner_counter: "Presentar en mostrador o taquilla",
  contact_operations: "Contactar a Operaciones",
  operations_delivery: "Entrega registrada por Operaciones",
};
function openRewardEditor(reward = null) {
  ...
  bindForm("#reward-editor", async (values, form) => {
    ...
    await rpc("upsert_reward", {
      ...
      min_referrals_registered: Number(values.min_referrals_registered || 0),
      min_referrals_first_trip: Number(values.min_referrals_first_trip || 0),
      audience: values.audience,
      delivery_mode: values.delivery_mode,
      claim_method: values.claim_method,
      claim_instructions: values.claim_instructions,
      claim_contact: values.claim_contact,
      claim_contact_url: values.claim_contact_url,
      claim_location: values.claim_location,
      claim_button_label: values.claim_button_label,
      name: values.name,
...
function rewardOperationCard(redemption) {
  const external = redemption.delivery_mode !== "trip";
...
function openRewardOperationReview(redemption, result) {
  const labels = {
    fulfilled: ["Preparar recompensa", "Registra la referencia de preparación o entrega parcial. El ticket quedará disponible para la persona.", "Marcar lista"],
    redeemed: ["Registrar canje", "Confirma que el negocio u Operaciones validó el beneficio. El código quedará inutilizable.", "Confirmar canje"],
    cancelled: ["Cancelar recompensa", "Los puntos se devolverán automáticamente y el folio quedará cancelado.", "Cancelar y devolver puntos"],
  };
  const [title, description, submit] = labels[result];
  const needsReference = result !== "cancelled";
  openModal(
    title,
    `<form id="reward-review"><p>${e(description)}</p>${needsReference ? `<label>Referencia o comprobación<input name="reference" required minlength="3" maxlength="120" placeholder="Folio del negocio, recarga o entrega"></label>` : ""}<label>Nota de Operaciones<textarea name="note" required minlength="5" maxlength="500" placeholder="Describe la validación realizada y cualquier dato útil."></textarea></label><p class="hint">${e(redemption.user_name || "Usuario")} · ${e(redemption.name)} · ${e(redemption.code)}</p><button class="btn wide ${result === "cancelled" ? "danger" : ""}" type="submit">${e(submit)}</button></form>`,
  );
  bindForm("#reward-review", async (values) => {
    ...
    closeModal();
    await refreshPage();
    notify(result === "cancelled" ? "Canje cancelado y puntos devueltos." : result === "redeemed" ? "Canje registrado; el código ya no puede reutilizarse." : "Recompensa lista para canjear.");
  });
}
function bindRewardOperationControls(redemptions, root = document) {
  $$('[data-ops-reward-ticket]', root).forEach((item) => item.onclick = () => {
    const redemption = redemptions.find((entry) => entry.id === item.dataset.opsRewardTicket);
    if (redemption) openRewardCoupon(redemption);
  });
  $$('[data-reward-review]', root).forEach((item) => item.onclick = () => {
    const redemption = redemptions.find((entry) => entry.id === item.dataset.rewardReview);
...
function marketingView() {
  ...
  const referrals = S.data.reward_operations?.referrals || [];
  const referralSummary = S.data.reward_operations?.referral_summary || {};
  ...
  const filterRedemptions = () => {
    const audience = $("#reward-ops-audience").value;
    const status = $("#reward-ops-status").value;
    let visible = 0;
    $$('[data-reward-op-audience]').forEach((card) => {
      const show = (audience === "all" || card.dataset.rewardOpAudience === audience)
        && (status === "all" || card.dataset.rewardOpStatus === status);
      card.classList.toggle("hidden", !show);
      if (show) visible += 1;
    });
    $("#reward-ops-count").textContent = `${visible} registro${visible === 1 ? "" : "s"}`;
  };
  $("#reward-ops-audience").onchange = filterRedemptions;
  $("#reward-ops-status").onchange = filterRedemptions;
...
function safety() {
 const driver=S.profile.role==="driver";
 shell(`<section class="panel safety-intro"><span class="safety-mark">${I("shield-check")}</span><div><div class="eyebrow">SEGURIDAD YAVOI!</div><h2>Viaja con información y pide ayuda cuando la necesites</h2><p>Confirma los datos del viaje y usa los controles de Yavoi! durante el servicio. Si hay peligro inmediato, llama al 911.</p><a class="btn danger" href="tel:911">${I("phone-call")} Emergencias 911</a></div></section><div class="grid2 section-gap"><section class="panel safety-step"><h3>${I("clipboard-check")} Antes del viaje</h3><ul><li>Revisa nombre y fotografía de la otra persona, vehículo, color y placas mostrados en el viaje.</li><li>Si los datos no coinciden, no inicies el viaje; cancela y repórtalo a Operaciones.</li><li>Comparte el estado del viaje con alguien de confianza y acuerda a dónde vas.</li>${driver?"<li>Confirma el destino y la forma de pago antes de aceptar; mantén actualizados tus datos de unidad.</li>":"<li>No compartas el PIN de inicio hasta estar dentro de la unidad correcta.</li>"}</ul></section><section class="panel safety-step"><h3>${I("map")} Durante el viaje</h3><ul><li>Usa cinturón y verifica que la ruta y el destino sean los acordados.</li><li>Si hay un cambio de ruta que no reconoces, pide una explicación y comunícate con alguien de confianza.</li><li>Ante una detención inesperada, acoso o sensación de riesgo, busca un lugar público seguro y llama al 911 si el peligro es inmediato.</li><li>El reporte dentro de la app sirve para seguimiento; no sustituye una llamada de emergencia.</li></ul></section><section class="panel safety-step"><h3>${I("message-square-warning")} Si algo ocurre</h3><ul><li>Aléjate a un lugar seguro. En una emergencia, llama al 911.</li><li>Después, abre Ayuda y registra el viaje, lo sucedido y si necesitas atención prioritaria.</li><li>Conserva folios y comprobantes relacionados con el servicio.</li></ul><a class="btn secondary" href="#help">${I("life-buoy")} Contactar a Yavoi!</a></section><section class="panel safety-step"><h3>${I("badge-check")} Compromiso de seguridad</h3><p>Operaciones revisa la identidad, documentos y unidad antes de autorizar a un conductor. Confirma siempre que la información visible en Yavoi! coincida con la persona y el vehículo presentes.</p><p>Yavoi! registra el estado y la información del viaje para facilitar el seguimiento de reportes. La aplicación no reemplaza servicios de emergencia ni garantiza respuesta inmediata.</p><div class="policy-links"><a class="link" href="/terminos.html">Términos de servicio ${I("arrow-up-right")}</a><a class="link" href="/privacidad.html">Privacidad ${I("arrow-up-right")}</a></div></section></div>`,"Seguridad","Recomendaciones antes, durante y después de cada viaje.");
}
async function inbox() {
 const result=await inboxRpc("list"),messages=result.messages||[],unread=messages.filter(x=>!x.is_read).length;
 const category={general:"Aviso",safety:"Seguridad",payment:"Pagos",service:"Servicio",promotion:"Promoción",news:"Novedades"};
 const cards=messages.length?messages.map(x=>`<details class="inbox-message ${x.is_read?"":"unread"}" data-inbox-item="${e(x.id)}"><summary><span class="inbox-category">${I(x.category==="safety"?"shield-check":x.category==="payment"?"wallet":"bell")}</span><span class="inbox-message-title"><strong>${e(x.title)}</strong><small>${e(category[x.category]||"Aviso")} · ${date(x.created_at)}</small></span>${x.is_read?'<span class="badge neutral">Leído</span>':'<span class="badge pending">Nuevo</span>'}</summary><div class="inbox-message-body"><p>${e(x.body)}</p>${x.ends_at?`<small>Vigente hasta ${date(x.ends_at)}</small>`:""}</div></details>`).join(""):'<div class="empty"><p>Tu bandeja está al día. Aquí aparecerán los avisos y mensajes de Yavoi!.</p></div>';
 const admin=S.profile.role==="admin";
 const sent=admin&&result.sent?.length?`<section class="panel section-gap"><h2>Comunicados recientes</h2><div class="inbox-list">${result.sent.map(x=>`<article class="inbox-sent"><div><strong>${e(x.title)}</strong><small>${e(category[x.category]||"Aviso")} · ${x.audience==="all"?"Todos":x.audience==="passenger"?"Pasajeros":"Conductores"}</small></div><span class="badge neutral">Hasta ${date(x.ends_at)}</span></article>`).join("")}</div></section>`:"";
 shell(`${admin?`<section class="panel broadcast-panel"><div class="row between wrap"><div><div class="eyebrow">CENTRO DE OPERACIÓN</div><h2>Enviar comunicado</h2><p>Publica un aviso en la bandeja Yavoi! de todos los usuarios o de un grupo.</p></div>${I("send")}</div><form id="broadcast-form" class="broadcast-form"><div class="grid2"><label>Destinatarios<select name="audience"><option value="all">Todos los usuarios</option><option value="passenger">Pasajeros</option><option value="driver">Conductores</option></select></label><label>Tipo de mensaje<select name="category"><option value="general">Aviso general</option><option value="safety">Seguridad</option><option value="payment">Pagos</option><option value="service">Servicio</option><option value="promotion">Promoción</option><option value="news">Novedades</option></select></label></div><label>Título<input name="title" required minlength="3" maxlength="120" placeholder="Escribe un título claro"></label><label>Mensaje<textarea name="body" required minlength="5" maxlength="2000" placeholder="Comparte la información que necesitan conocer."></textarea></label><label>Vigente hasta<input name="ends_at" type="datetime-local" required></label><button class="btn" type="submit">${I("send")} Publicar comunicado</button><p class="hint">Publicar requiere el acceso de Operaciones con verificación en dos pasos. Cada envío queda registrado para auditoría.</p></form></section>${sent}`:`<div class="inbox-summary panel"><div><div class="eyebrow">MENSAJES Y AVISOS</div><h2>${unread?`${unread} mensaje${unread===1?"":"s"} nuevo${unread===1?"":"s"}`:"Estás al día"}</h2><p>Comunicados de Operaciones, novedades de servicio y avisos importantes.</p></div>${I("inbox")}</div>`}<section class="panel section-gap"><div class="row between wrap"><div><h2>${admin?"Vista de bandeja":"Bandeja de Entrada"}</h2><p>Los avisos vigentes se actualizan automáticamente.</p></div><button class="btn secondary" type="button" data-action="refresh">${I("refresh-cw")} Actualizar</button></div><div class="inbox-list section-gap">${cards}</div></section>`,"Bandeja de Entrada",admin?"Publica y revisa avisos para usuarios de Yavoi!.":"Mensajes de Yavoi! y novedades relacionadas con tu cuenta.");
 $$('[data-inbox-item]').forEach(item=>item.addEventListener("toggle",()=>{if(item.open&&item.classList.contains("unread"))run(async()=>{await inboxRpc("read",{announcement_id:item.dataset.inboxItem});item.classList.remove("unread");const b=item.querySelector(".badge");if(b){b.className="badge neutral";b.textContent="Leído";}})}));
 if(admin)bindForm("#broadcast-form",async values=>{await inboxRpc("publish",{...values,ends_at:new Date(values.ends_at).toISOString()});notify("Comunicado publicado en las bandejas seleccionadas.");await inbox();});
}
function help() {
  const admin = S.profile.role === "admin";
...
function fleet() {
  ...
  bindRewardOperationControls(rewardOperations.pending || []);
}
function rates() {
  ...
  const shiftEditor = `<section class="panel section-gap shift-editor"><div class="row between wrap"><div><div class="eyebrow">CONTROL DE OPERACIONES</div><h2>Turnos de servicio</h2><p>Operaciones define estos horarios y asigna el turno individual desde Conductores y flotilla. Los conductores no pueden elegirlo ni cambiarlo. El turno Nocturno cruza la medianoche y Todo el día disponible permite conectarse a cualquier hora.</p></div><span class="badge neutral">Hora de Chihuahua</span></div><div class="shift-grid">${(S.data.service_shifts || []).map((shift) => { const allDay = shift.code === "all_day"; return `<form data-service-shift="${e(shift.code)}"><div><strong>${e(shift.name)}</strong><small>${e(shiftTimeLabel(shift))}</small></div><label>Nombre<input name="name" required maxlength="40" value="${e(shift.name)}" ${allDay ? "readonly" : ""}></label><label>Inicio<input name="start_time" type="time" required value="${e(String(shift.start_time).slice(0, 5))}" ${allDay ? "readonly" : ""}></label><label>Fin<input name="end_time" type="time" required value="${e(String(shift.end_time).slice(0, 5))}" ${allDay ? "readonly" : ""}></label><label class="check"><input name="active" type="checkbox" ${shift.active ? "checked" : ""}>Disponible para asignar</label><button class="btn secondary" type="submit">${allDay ? "Guardar disponibilidad" : "Guardar horario"} ${I("save")}</button></form>`; }).join("")}</div></section>`;
  shell(
