-- Securely persist a suggested street route for trips created before route snapshots
-- or confirmed while the routing response was still pending in the client.
create function private.capture_visible_trip_route_v1(payload jsonb) returns jsonb
language plpgsql security definer set search_path='' as $$
declare
  uid uuid:=auth.uid();
  profile_value public.profiles;
  trip_value public.trips;
  target uuid:=nullif(payload->>'trip_id','')::uuid;
begin
  select * into profile_value from public.profiles where id=uid;
  if uid is null or not found or profile_value.suspended then
    raise exception 'Inicia sesión para continuar.' using errcode='42501';
  end if;
  if profile_value.role='admin' and coalesce(auth.jwt()->>'aal','aal1')<>'aal2' then
    raise exception 'Operaciones requiere verificación en dos pasos.' using errcode='42501';
  end if;
  if payload is null or jsonb_typeof(payload)<>'object' or octet_length(payload::text)>350000 then
    raise exception 'Ruta inválida.';
  end if;
  select * into trip_value from public.trips where id=target;
  if not found or (trip_value.passenger_id is distinct from uid and trip_value.driver_id is distinct from uid and profile_value.role<>'admin') then
    raise exception 'No tienes acceso a este viaje.' using errcode='42501';
  end if;
  perform private.capture_trip_route_v1(target,payload->'planned_route');
  select * into trip_value from public.trips where id=target;
  if trip_value.planned_route is null then raise exception 'No fue posible validar la ruta sugerida.';end if;
  if not exists(select 1 from public.trip_events where trip_id=target and event='planned_route_saved') then
    insert into public.trip_events(trip_id,actor_id,event,detail)
      values(target,uid,'planned_route_saved',jsonb_build_object('provider',trip_value.planned_route_provider));
  end if;
  return jsonb_build_object(
    'coordinates',coalesce(trip_value.planned_route->'coordinates','[]'::jsonb),
    'distance_km',trip_value.planned_route_distance_km,
    'duration_minutes',trip_value.planned_route_duration_minutes,
    'provider',trip_value.planned_route_provider,
    'created_at',trip_value.planned_route_created_at,
    'instructions',coalesce(trip_value.planned_route->'instructions','[]'::jsonb)
  );
end $$;
revoke all on function private.capture_visible_trip_route_v1(jsonb) from public,anon;
grant execute on function private.capture_visible_trip_route_v1(jsonb) to authenticated;
