...
begin
 ...
 if command='dashboard' then
  ...
  if command='trip' then
   ...
   if (uid is distinct from t.passenger_id and uid is distinct from t.driver_id) or t.driver_id is null or t.status in ('completed','cancelled') then raise exception 'El chat sólo está disponible durante un viaje asignado.';end if;
   ...
   if t.status<>'completed' or (uid is distinct from t.passenger_id and uid is distinct from t.driver_id) then raise exception 'Sólo los participantes pueden evaluar un viaje completado.';end if;
